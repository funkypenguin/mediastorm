...
// Note: This uses context.Background() for compatibility with afero.Fs interface.
// For request-scoped contexts, use OpenWithContext instead.
...
// OpenWithContext opens a file for reading with a provided context.
// This allows proper cancellation when the HTTP request context is cancelled.
func (nfs *NzbFilesystem) OpenWithContext(ctx context.Context, name string) (afero.File, error) {
	// Parse path with args
	pr, err := utils.NewPathWithArgsFromString(name)
	if err != nil {
		return nil, err
	...
	// Try to open with NZB remote file, passing through the request context
	ok, file, err := nfs.remoteFile.OpenFile(ctx, pr.Path, pr)
	if err != nil {
		return nil, err
	}
