...
func TestGetRequestRangeUsesSeekPosition(t *testing.T) {
	fileSize := int64(50 * 1024 * 1024)
	seekPos := int64(5 * 1024 * 1024)

	mvf := createTestVirtualFile(fileSize)
	mvf.position = seekPos
	mvf.originalRangeEnd = -1

	start, end := mvf.getRequestRange()

	if start != seekPos {
		t.Fatalf("expected range start %d, got %d", seekPos, start)
	}

	if end != -1 {
		t.Fatalf("expected unbounded range end (-1), got %d", end)
	}
}

func TestGetRequestRangeRespectsOriginalEnd(t *testing.T) {
	fileSize := int64(20 * 1024 * 1024)
	originalEnd := int64(10 * 1024 * 1024)
	seekPos := int64(8 * 1024 * 1024)
	...
	if end != originalEnd {
		t.Fatalf("expected range end %d, got %d", originalEnd, end)
	}
}

func TestGetRequestRangeClampsWhenPastOriginalEnd(t *testing.T) {
	fileSize := int64(20 * 1024 * 1024)
	originalEnd := int64(10 * 1024 * 1024)
	seekPos := originalEnd + 5*1024
	...
	if start != originalEnd {
		t.Fatalf("expected clamped range start %d, got %d", originalEnd, start)
	}

	if end != originalEnd {
		t.Fatalf("expected clamped range end %d, got %d", originalEnd, end)
	}
}
