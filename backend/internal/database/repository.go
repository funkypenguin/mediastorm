...
// TryClaimQueueItem attempts to transition a specific item into processing if it is pending/retrying.
func (r *Repository) TryClaimQueueItem(id int64) (bool, error) {
	now := time.Now()
	...
	rowsAffected, err := result.RowsAffected()
	if err != nil {
		return false, fmt.Errorf("failed to retrieve affected rows claiming queue item %d: %w", id, err)
	}
...
// UpdateQueueMetadata sets metadata JSON for a queue item.
func (r *Repository) UpdateQueueMetadata(id int64, metadata *string) error {
	query := `
		UPDATE import_queue
	...
	if _, err := r.db.Exec(query, metadata, id); err != nil {
