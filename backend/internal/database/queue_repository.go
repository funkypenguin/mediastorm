...
// UpdateMetadata updates the metadata JSON for a queue item.
func (r *QueueRepository) UpdateMetadata(itemID int64, metadata *string) error {
	query := `
		UPDATE import_queue
	...
	if _, err := r.db.Exec(query, metadata, itemID); err != nil {
		return fmt.Errorf("failed to update queue metadata: %w", err)
	}
...
// TryClaimQueueItem attempts to mark a specific item as processing when it is still pending or retrying.
// Returns true when the transition succeeded.
func (r *QueueRepository) TryClaimQueueItem(id int64) (bool, error) {
	now := time.Now()
	...
	if err != nil {
		return false, fmt.Errorf("failed to claim queue item %d: %w", id, err)
	}

	rowsAffected, err := result.RowsAffected()
	if err != nil {
		return false, fmt.Errorf("failed to check rows affected when claiming queue item %d: %w", id, err)
	}
