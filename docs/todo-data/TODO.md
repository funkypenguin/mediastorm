...
- In Progress Testing — Configured-settings browsers: on mobile, verify the near-full-width cards, hierarchy, directional slides, search, and long values; on tvOS and Fire TV, verify scope → category → section → item navigation slides in the expected direction, Back reverses direction, focus never escapes or disappears, source/read-only badges are accurate, long values remain readable, and reduced-motion disables transitions
